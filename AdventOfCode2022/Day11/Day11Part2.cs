...
namespace AdventOfCode2022.Day11
{
    class Day11Part2 : AocDay
    {
        public async Task Run()
        {
            ...
            var inspections = CountInspections(state, 10000);
        ...
        long CountInspections(ProblemState state, long rounds)
        {
            ...
            return topTwo.Aggregate((long)1, (a, b) => a * b);
        }
            ...
            for (long round=1; round<=rounds; round++)
            {
                foreach (var monkey in state.Monkeys.Values)
                {
                    while (monkey.Items.Any())
                    {
                        ...
                        long relievedWorryLevel = nextWorryLevel % commonDivisor;
